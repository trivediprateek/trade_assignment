...
def health_check():
    ...
    return {
        "status": "healthy",
        "message": "Trade Store API is running"
    }
...
def create_trade(trade: TradeCreate):
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Failed to queue trade: {str(e)}"
        )
...
def update_trade(trade_id: str, version: int, trade_update: TradeUpdate):
    ...
    message_data = {
        ...
        "data": {
            "trade_id": trade_id,
            "version": version,
            **trade_update.model_dump(exclude_unset=True, mode='json')
        }
    }
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to queue update: {str(e)}"
        )
...
def mark_expired_trades(db: Session = Depends(get_db)):
    ...
    count = TradeService.mark_expired_trades(db)
    return {
        "message": f"Marked {count} trades as expired",
        "count": count
    }
