...
def delivery_report(err, msg):
    ...
    if err is not None:
        ...
        logger.info(f" Message delivered to {msg.topic()} " f"[partition {msg.partition()}] at offset {msg.offset()}")
...
def create_topic_if_not_exists(topic: str):
    ...
    new_topic = NewTopic(topic, num_partitions=3, replication_factor=1)  # Multiple partitions for parallel processing
...
def send_sample_trades(count: int):
    ...
    try:
        for i in range(1, count + 1):
            ...
            producer.produce(
                topic, value=message.encode("utf-8"), key=trade_id.encode("utf-8"), callback=delivery_report
            )
...
def send_continuous_trades(delay: float = 0.1):
    ...
    try:
        while True:
            ...
            message = json.dumps(message_data)
            producer.produce(topic, value=message.encode("utf-8"), key=trade_id.encode("utf-8"))
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description="Send sample trades to Kafka for testing")

    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--count", type=int, help="Number of sample trades to send")
    group.add_argument("--continuous", action="store_true", help="Send trades continuously (stress test)")
    group.add_argument("--file", type=str, help="Send trades from JSON file")

    parser.add_argument("--delay", type=float, default=0.1, help="Delay between messages in continuous mode (seconds)")
